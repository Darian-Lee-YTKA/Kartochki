                    ...
                    ScrollView {
                        VStack {
                            ForEach(databaseManager.decks, id: \.self) { deck in
                                DeckRowView(currentView: $currentView, bindedLanguage: $language, databaseManager: $databaseManager, isShowingDeleteDeckAlert: $isShowingDeleteDeckAlert, edit: true, inLanguages: true, deck: deck)
                                    .padding(.vertical, 15)
                                    .frame(width: geometry.size.width - 10)
                            }
                            Spacer()
                            ForEach(databaseManager.nonLanguages, id: \.self) { nonlanguage in
                                DeckRowView(currentView: $currentView, bindedLanguage: $language, databaseManager: $databaseManager, isShowingDeleteDeckAlert: $isShowingDeleteDeckAlert, edit: true, inLanguages: false, deck: DeckData(name: nonlanguage, count: 0, dueCount: 0))
                                    .padding(.vertical, 15)
                                    .frame(width: geometry.size.width - 10)
                            }
                        }
